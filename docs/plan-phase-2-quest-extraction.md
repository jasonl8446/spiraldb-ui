...
- [ ] Extraction UI matches spec states, evidenced per D23 tier 2 (playwright-mcp assertions + screenshots copied into `docs/evidence/phase-2/`, including a 375px viewport check): drop zone copy includes the supported-format line; spinner is indeterminate with "Extracting quests..."; results split layout with confirm dialog before Save All; mobile shows preview as overlay.
- [ ] **UI test suite (D23 tier 1)**: committed `tests/ui/extraction.spec.ts` passes headless in CI — upload a D28-generated capture via the real dropzone input → spinner → results list shows N quests → Save All + confirm → success toast → browse list grows by N.
