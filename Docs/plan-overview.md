...
2. **Approve this plan** (status line above flips to `approved`).
...
- **D24 — Minimal CI.** `.github/workflows/ci.yml` created in Phase 1: on PR → `npm ci && npm test && npm run build && npm run test:ui` (headless chromium; workflow runs `npx playwright install chromium` first). Objective signal for async PR review.
