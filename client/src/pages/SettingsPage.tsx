 ...
 *
 * 1. the three configured paths plus the user name, saved with one `PUT`, and the
 *    read-only `git_branch` information row (D42's forward gap, closed in story
 *    p2-09 — see the inline comment);
    ...
    <div className="mx-auto flex max-w-4xl flex-col gap-6">
      <Card>
        ...
        <CardContent className="flex flex-col gap-4 pt-4">
          ...
          {/*
            D42's forward gap, closed here (story p2-09). `git_branch` is one of the
            five keys `GET /api/settings` returns (D32) but rendered nowhere before
            this story, even though the Phase 2 save path commits to exactly this
            branch (docs/spec-data-model.md L206-214).

            **Read-only on purpose.** The branch is generated by the save pipeline
            (`content/YYYY-MM-DD`) and created from `main` HEAD when it does not
            exist; a hand-edited value would silently redirect every subsequent
            commit away from the strategy the spec fixes, with no validation the UI
            could do better than the server. `PUT /api/settings` accepts the key
            (D32), so the UI is the only thing withholding it — this row never joins
            `edits`, so `Save` cannot send it. Recorded as a deviation from
            docs/spec-data-model.md L214 ("User can change branch in Settings page")
            in the story evidence.
          */}
          <div className="flex flex-col gap-1.5">
            <label htmlFor="git_branch" className="text-sm font-medium text-zinc-300">
              Git Branch
            </label>
            <Input
              id="git_branch"
              name="git_branch"
              value={settings.data?.git_branch ?? ''}
              readOnly
              spellCheck={false}
              autoComplete="off"
